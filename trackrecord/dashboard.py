...
/* A fixed track minimum is wider than a phone, so the track itself is capped at the
   space available before the column count is decided. */
...
/* A grid or flex item takes its min-content as an automatic minimum, so a single
   unbreakable value — or a wide table — stretches its track and drags the whole page
   sideways. Nothing laid out here needs that minimum, at any width. */
