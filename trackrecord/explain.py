...
NOTES: list[tuple[str | None, str, str, str]] = [
    ...
    (r"construction|/active", r"^downside capture$",
     "In every month the benchmark fell, the portfolio's average return is divided by the benchmark's average return. A figure below 100% means the book fell less than the benchmark when the benchmark was down. The upside figure beside it is the same calculation on months the benchmark rose.",
     "This is how much of the benchmark's pain the process absorbed, which is the plain answer to whether the book protected on the downside. It is measured against the same managers' aggregate book as the active return, not against the market."),
